...
#region | Ribbon Buttons |
  ...
  public void CopyVisibleCells() {
    ...
    try {
      ...
      visibleRange = Globals.ThisAddIn.Application.Selection.SpecialCells(
  ...
  public void CreateFileList() {
    ...
    try {
      ...
      if (msgDialogResult == DialogResult.OK |
          Properties.Settings.Default.Option_PathFileListingSelect == false) {
        filePath += @"\";
        string scriptCommands = string.Empty;
        string currentDate = DateTime.Now.ToString("dd.MMM.yyyy_hh.mm.tt");
        string batchFileName = filePath + "FileListing_" + currentDate + "_" +
                               Environment.UserName + ".bat";
        scriptCommands = "echo off" + Environment.NewLine;
        scriptCommands += "cd %1" + Environment.NewLine;
        scriptCommands +=
            @"dir """ + filePath + @""" /s /a-h /b /-p /o:gen >""" + filePath +
            "FileListing_" + currentDate + "_" + Environment.UserName +
            @".csv""" + Environment.NewLine;
        scriptCommands += @"""" + filePath + "FileListing_" + currentDate +
                          "_" + Environment.UserName + @".csv""" +
                          Environment.NewLine;
      ...
      MessageBox.Show("You don't have access to this folder, bro!" +
  ...
  public void OpenNewIssue() {
    try {
      System.Diagnostics.Process.Start(
          Properties.Settings.Default.App_PathReportIssue);
  ...
  public void OpenSheetVisibility() {
    try {
      if (myTaskPaneWorksheetList != null) {
        ...
        myTaskPaneWorksheetList = Globals.ThisAddIn.CustomTaskPanes.Add(
            myWorksheetList, "Worksheet List");
        myTaskPaneWorksheetList.DockPosition =
            Office.MsoCTPDockPosition.msoCTPDockPositionRight;
        myTaskPaneWorksheetList.DockPositionRestrict =
